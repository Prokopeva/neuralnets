...
def build_conv_layer() -> tf.keras.layers.Conv2D:
    ...
    my_layer = tf.keras.layers.Conv2D(2, (5, 5), 5)
...
def build_padded_conv_layer(kernel_size) -> tf.keras.layers.Conv2D:
    ...
    my_layer = tf.keras.layers.Conv2D(2, kernel_size, 1, 'same')
...
def build_depth_wise_conv_layer() -> tf.keras.layers.DepthwiseConv2D:
    ...
    my_layer = tf.keras.layers.DepthwiseConv2D((3,3), depth_multiplier=2)
    return my_layer
...
def build_pooling_layer() -> tf.keras.layers.MaxPooling2D:
    ...
    my_layer = tf.keras.layers.MaxPooling2D((3,3), 1)
    return my_layer
...
def build_up_conv_layer() -> tf.keras.layers.Conv2DTranspose:
    ...
    my_layer = tf.keras.layers.Conv2DTranspose(4, (3,3), 2)
    return my_layer
